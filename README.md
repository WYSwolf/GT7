...
想擴充成「**我擅長什麼**」的長期剖面,需要先在資料上補標籤:

- **依車種/車型分析**:每個組合標上 **車種(carClass)/ 傳動(MR/FR/FF/4WD)/ 級別(Gr.3、道路…)**,
  再把各組合的「vs WR %」「百分位」「一致性」**依車種聚合**,看出
  「你開 MR 中置引擎特別快」「Gr.3 比道路車強」這類傾向。
- **依賽道類型分析**:標上 **賽道屬性(高速/技術彎、長度、順/逆向)**,聚合後看
  「高速賽道你較吃香」「多低速彎的賽道相對弱」。
- **呈現**:一張「強項雷達/排行」—— 各車種與賽道類型的平均 vs-WR%,排序出強弱。
- **前提**:目前資料量(少數組合)還不足以下定論;每新增一個賽道/車組合,
  就累積一筆,等樣本夠了再開這個分析頁。屆時只要在 `sessions`/`meta` 補上述標籤即可。
